...
import { api, Provider, SERVICE_CATEGORIES, isProviderOpen } from "@/lib/api";
...
export default function Home() {
  ...
  const [stats, setStats] = useState({ providers: 0, customers: 0, services: 0 });
  const [, navigate] = useLocation();

  useEffect(() => {
    api.getProviders().then(all => {
      setProviders(all);
      setStats({
        providers: all.length,
        customers: 0,
        services: new Set(all.map(p => p.subCategory)).size,
      });
    }).catch(() => {});
  }, []);
  ...
  const handleSearch = async () => {
    setLoading(true);
    try {
      const results = await api.getProviders(pinCode.trim() || undefined, selectedCategory || undefined);
      setFiltered(results);
      setSearched(true);
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
      <section className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-800 text-white py-20 px-4">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <div className="inline-flex items-center gap-2 bg-white/20 rounded-full px-4 py-2 text-sm mb-6 backdrop-blur-sm">
            ...
            <span>Verified local service providers across India</span>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-2xl max-w-2xl mx-auto">
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Button onClick={handleSearch} disabled={loading} className="h-12 px-6 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl gap-2">
      ...
      <section className="bg-white border-b border-gray-100 py-6">
        <div className="max-w-5xl mx-auto px-4 grid grid-cols-3 gap-6 text-center">
          {[
            { icon: <Users className="w-5 h-5" />, label: "Active Providers", value: stats.providers },
            { icon: <ThumbsUp className="w-5 h-5" />, label: "Service Categories", value: Object.keys(SERVICE_CATEGORIES).length },
            { icon: <Shield className="w-5 h-5" />, label: "Verified Services", value: stats.services },
          ].map(stat => (
      ...
      <section className="py-12 px-4 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          {Object.entries(SERVICE_CATEGORIES).map(([cat, services]) => (
            <button
              key={cat}
              onClick={async () => { setSelectedCategory(cat); const r = await api.getProviders(undefined, cat); setFiltered(r); setSearched(true); }}
        ...
        <section className="pb-16 px-4 max-w-5xl mx-auto">
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-16 text-gray-500">
              ...
              <p className="text-sm mt-1">Try a different PIN code or category, or check back soon.</p>
            </div>
          ) : (
