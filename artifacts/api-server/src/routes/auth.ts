...
router.post("/auth/register", async (req, res) => {
  try {
    ...
    const user = { id: genId(), name, email, password, phone, role: "user" as const };
...
router.post("/auth/login", async (req, res) => {
  try {
    ...
    if (email === "admin@quickseva.com" && password === "admin123") {
      return res.json({ success: true, user: { id: "admin", name: "Admin", email, phone: "", role: "admin" } });
    }
...
router.post("/auth/provider/register", async (req, res) => {
  try {
    const { ownerName, shopName, email, password, phone, category, subCategory, address, pinCode, openingTime, closingTime } = req.body;
